...
use ory_client::{
    apis::{configuration::Configuration, identity_api, frontend_api},
    models::{Identity, Session, UpdateLoginFlowBody},
};
...
pub async fn api_login(
    ...
    let refreshable_x_session = match &*body {
        LoginRequest::Password { refreshable_x_session, .. } => refreshable_x_session,
        LoginRequest::Oauth2 { refreshable_x_session, .. } => refreshable_x_session,
    };

    // Create login flow
    let flow = frontend_api::create_native_login_flow(&configuration, Some(refreshable_x_session.is_some()), None, refreshable_x_session.as_deref()).await.map_err(OryError::from)?;
    ...
    let update_login_flow_body = match &*body {
        ...
        LoginRequest::Oauth2 { provider, email, .. } => {
            UpdateLoginFlowBody::UpdateLoginFlowWithOidcMethod { 
                csrf_token: None, 
                provider: provider.to_string(), 
                traits: Some(serde_json::to_value(ImportedUserTraits {
                    email: email.clone(),
    ...
    let flow = frontend_api::update_login_flow(&configuration, &flow.id, &update_login_flow_body, refreshable_x_session.as_deref(), None).await.map_err(OryError::from)?;
