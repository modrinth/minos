...
import { extract_nested_csrf_token, extract_nested_error_messages_from_error} from "~/helpers/ory-ui-extract"
...
let password = ref("");
  ...
  async function registerPassword() {
    ...
    registerGeneric(registrationFlowBody)
  }
  ...
  let flow_data = await $oryConfig.getRegistrationFlow({id:route.query.flow})

  // Directly extract csrf_token from nested returned Ory UI elements
  registrationFlowBody.csrf_token = extract_nested_csrf_token(flow_data.data);
  ...
  await $oryConfig.updateRegistrationFlow({
    ...
    updateRegistrationFlowBody: registrationFlowBody
        ...
        let return_url = flow_data.data.return_to || config.nuxtUrl;
